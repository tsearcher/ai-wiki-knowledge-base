# 可直接导入的完整 Skill 文件包
以下是 `project-packager` Skill 的全部文件内容，你只需按目录结构创建对应文件夹，将内容保存为同名文件，即可直接导入 Skill 系统使用。
...
├── SKILL.md                          # 主控引导文件（必选）
└── references/                       # 资源目录
    ├── packager-naming-rules.md      # 命名规范与排除规则
    ├── packager-script-python.md     # Python 打包脚本（集成Wiki录入）
    ├── packager-script-shell.md      # Shell 打包脚本（纯打包用）
    ├── wiki-structure.md             # 本地AI Wiki 结构规范与模板
    └── wiki-manager-script.py        # Wiki 管理核心脚本
```

---

## 1. SKILL.md（主控文件）
保存为 `project-packager/SKILL.md`
```markdown
---
...
## 概述
本 Skill 用于软件版本发布、归档备份与知识库管理场景。核心能力包含两部分：
...
2. 读取命名规范：读取 references/packager-naming-rules.md，遵循命名约定
3. 执行打包逻辑：读取 references/packager-script-python.md，生成定制化脚本
4. 读取Wiki规范：读取 references/wiki-structure.md，遵循结构与模板
...
2. 读取Wiki结构规范：读取 references/wiki-structure.md
3. 调用Wiki管理脚本：读取 references/wiki-manager-script.py，执行对应操作
4. 返回操作结果与验证信息
...
## 2. references/packager-naming-rules.md
保存为 `project-packager/references/packager-naming-rules.md`
```markdown
# 打包命名规范与排除规则
...
| 字段 | 规则 | 示例 |
|------|------|------|
| 项目名 | 全小写，单词用连字符分隔 | `user-service`, `admin-web` |
| 版本标识 | 前缀 `v` + 语义化版本号 | `v1.2.3`, `v2.0.0-beta.1` |
| 日期 | 8 位数字 YYYYMMDD | `20260821` |
| 类型标识 | release / debug / hotfix / snapshot | `release`, `hotfix` |
| 扩展名 | 压缩格式后缀 | `zip`, `tar.gz` |

### 完整示例
- `user-service_v1.2.3_20260821_release.zip`
- `admin-web_v2.0.0-beta_20260821_snapshot.tar.gz`
- `data-tool_v0.9.1_20260821_hotfix.zip`

## 二、版本号来源优先级

1. **命令行指定**：用户手动传入版本号（最高优先级）
2. **VERSION 文件**：项目根目录下的 VERSION / version.txt
3. **配置文件**：package.json / pyproject.toml / setup.py / Cargo.toml
...
| 格式 | 适用场景 | 优点 |
|------|----------|------|
| zip | 跨平台交付、Windows 用户多 | 兼容性最好，无需解压工具 |
| tar.gz | Linux 服务器、开源项目 | 压缩率高，保留文件权限 |
| 7z | 大项目、追求高压缩率 | 压缩率最高 |
```

---

## 3. references/packager-script-python.md
保存为 `project-packager/references/packager-script-python.md`
```markdown
# Python 打包脚本（集成Wiki自动录入）

跨平台通用，支持 Windows / macOS / Linux，打包完成后可自动同步版本信息到本地AI Wiki。
...
"""
项目打包归档脚本
自动生成带版本号和日期的压缩包，支持自动录入本地AI Wiki
"""
...
def pack_project(project_dir: str, output_dir: str = None, 
    ...
    with zipfile.ZipFile(output_path, 'w', zipfile.ZIP_DEFLATED) as zf:
        for root, dirs, files in os.walk(project_path):
            ...
            # 过滤排除目录（修改 dirs 列表影响 os.walk 遍历）
...
def main():
    ...
    parser.add_argument('-v', '--version', help='版本号（不指定则自动识别）')
    parser.add_argument('-e', '--exclude', action='append', help='额外排除的目录/文件')
...
```bash
# 基础打包（自动识别版本）
python pack.py ./my-project

# 指定版本号和输出目录
python pack.py ./my-project -v 1.2.3 -o ./releases
...
# 额外排除目录
python pack.py ./my-project -e docs -e tests
```
```

---

## 4. references/packager-script-shell.md
保存为 `project-packager/references/packager-script-shell.md`
```markdown
# Shell 打包脚本模板

适用于 Linux / macOS 环境，基于 tar / zip 命令，仅用于纯打包场景，不集成Wiki功能。
如需Wiki录入能力，请使用 Python 版本脚本。
...
# 输出文件名
ARCHIVE_NAME="${PROJECT_NAME}_v${VERSION}_${DATE}_release"
...
if [ "$FORMAT" = "tar.gz" ]; then
    ...
    eval "tar -czf '$OUTPUT_FILE' $EXCLUDE_ARGS -C '$PROJECT_DIR' ."
else
    OUTPUT_FILE="$OUTPUT_DIR/${ARCHIVE_NAME}.zip"
    eval "zip -rq '$OUTPUT_FILE' '$PROJECT_DIR/' $EXCLUDE_ARGS -x '*/\.*'"
fi
...
# 指定版本号
./pack.sh ./my-project 1.2.3

# 指定输出目录
./pack.sh ./my-project 1.2.3 ./releases
```
```

---

## 5. references/wiki-structure.md
保存为 `project-packager/references/wiki-structure.md`
```markdown
# 本地AI Wiki 结构规范与页面模板
...
## 6. references/wiki-manager-script.py
保存为 `project-packager/references/wiki-manager-script.py`
```python
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
...
### 1. 导入 Skill
1. 在你的 Skill 存放目录下创建 `project-packager` 文件夹
2. 将上述 `SKILL.md` 文件放入该文件夹
3. 在 `project-packager` 下创建 `references` 子文件夹
4. 将其余 5 个资源文件全部放入 `references` 文件夹
5. 重启 Skill 加载器即可识别使用
...
# 2. 打包项目并同步到Wiki
python project-packager/references/packager-script-python.md \
  ./你的项目目录 \
  -v 1.0.0 \
  --wiki-root ./local-ai-wiki \
  --changelog "首次正式发布"

# 3. 查询历史版本
python project-packager/references/wiki-manager-script.py \
  --wiki-root ./local-ai-wiki \
  query --name 你的项目名
```

需要我补充一键安装脚本或者 Windows 批处理版本的打包工具吗？